...
export class OrdonnanceService {
  ...
  public resourceSearchUrl = SERVER_API_URL + 'api/_search/ordonnances';
  ...
  search(req: Search): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IOrdonnance[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }

  protected convertDateFromClient(ordonnance: IOrdonnance): IOrdonnance {
    const copy: IOrdonnance = Object.assign({}, ordonnance, {
      date: ordonnance.date && ordonnance.date.isValid() ? ordonnance.date.toJSON() : undefined,
    });
    return copy;
  }
